...
- (void)renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    for (int i = 0; i < numActiveTrackables; ++i) {
        ...
        if (PLAYING != currentStatus) {
            ...
            glDrawElements(GL_TRIANGLES, kNumQuadIndices, GL_UNSIGNED_BYTE, quadIndices);
...
- (void)initShaders
{
    shaderProgramID = [SampleApplicationShaderUtils createProgramWithVertexShaderFileName:@"VivichR.vertsh"
                                                                fragmentShaderFileName:@"VivichR.fragsh"];
    
    if (0 < shaderProgramID) {
        vertexHandle = glGetAttribLocation(shaderProgramID, "a_position");
        normalHandle = glGetAttribLocation(shaderProgramID, "vertexNormal");
        textureCoordHandle = glGetAttribLocation(shaderProgramID, "a_vertexTexCoord");
        mvpMatrixHandle = glGetUniformLocation(shaderProgramID, "u_mvpMatrix");
        texSampler2DHandle  = glGetUniformLocation(shaderProgramID,"u_texture");
